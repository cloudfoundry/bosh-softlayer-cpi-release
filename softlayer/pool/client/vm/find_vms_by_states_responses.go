package vm

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/cloudfoundry/bosh-softlayer-cpi/softlayer/pool/models"
)

// FindVmsByStatesReader is a Reader for the FindVmsByStates structure.
type FindVmsByStatesReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *FindVmsByStatesReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewFindVmsByStatesOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	case 404:
		result := NewFindVmsByStatesNotFound()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result

	default:
		result := NewFindVmsByStatesDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	}
}

// NewFindVmsByStatesOK creates a FindVmsByStatesOK with default headers values
func NewFindVmsByStatesOK() *FindVmsByStatesOK {
	return &FindVmsByStatesOK{}
}

/*FindVmsByStatesOK handles this case with default header values.

successful operation
*/
type FindVmsByStatesOK struct {
	Payload *models.VmsResponse
}

func (o *FindVmsByStatesOK) Error() string {
	return fmt.Sprintf("[GET /vms/findByState][%d] findVmsByStatesOK  %+v", 200, o.Payload)
}

func (o *FindVmsByStatesOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.VmsResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewFindVmsByStatesNotFound creates a FindVmsByStatesNotFound with default headers values
func NewFindVmsByStatesNotFound() *FindVmsByStatesNotFound {
	return &FindVmsByStatesNotFound{}
}

/*FindVmsByStatesNotFound handles this case with default header values.

vm not found
*/
type FindVmsByStatesNotFound struct {
}

func (o *FindVmsByStatesNotFound) Error() string {
	return fmt.Sprintf("[GET /vms/findByState][%d] findVmsByStatesNotFound ", 404)
}

func (o *FindVmsByStatesNotFound) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewFindVmsByStatesDefault creates a FindVmsByStatesDefault with default headers values
func NewFindVmsByStatesDefault(code int) *FindVmsByStatesDefault {
	return &FindVmsByStatesDefault{
		_statusCode: code,
	}
}

/*FindVmsByStatesDefault handles this case with default header values.

unexpected error
*/
type FindVmsByStatesDefault struct {
	_statusCode int

	Payload *models.Error
}

// Code gets the status code for the find vms by states default response
func (o *FindVmsByStatesDefault) Code() int {
	return o._statusCode
}

func (o *FindVmsByStatesDefault) Error() string {
	return fmt.Sprintf("[GET /vms/findByState][%d] findVmsByStates default  %+v", o._statusCode, o.Payload)
}

func (o *FindVmsByStatesDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.Error)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
