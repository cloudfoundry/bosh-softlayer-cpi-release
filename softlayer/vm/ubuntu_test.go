...
import (
	...
	"github.com/cloudfoundry/bosh-softlayer-cpi/softlayer/vm/fakes"
...
var _ = Describe("UbuntuNetwork", func() {
	var (
		...
		networkComponents    vm.VirtualGuestNetworkComponents
	...
	BeforeEach(func() {
		...
		networkComponents = vm.VirtualGuestNetworkComponents{
			PrimaryBackendNetworkComponent: vm.NetworkComponent{
				Name:             "eth",
				Port:             0,
				PrimaryIPAddress: "10.155.248.190",
				NetworkVLAN: vm.NetworkVLAN{
					Name: "private vlan",
					Subnets: []vm.Subnet{{
			...
			PrimaryNetworkComponent: vm.NetworkComponent{},
		}
		softlayerClient = &fakes.FakeSoftLayerClient{}
		sshClient = &fakes.FakeSSHClient{}
		softlayerFileService = &fakes.FakeSLFileService{}
	...
	Describe("ConfigureNetwork", func() {
		...
		It("uploads the configuration and restarts networking", func() {
			...
			err := ubuntu.ConfigureNetwork(networks, fakes.NewFakeVM(1234567))
			Expect(err).NotTo(HaveOccurred())
		...
		Context("when a single dynamic network is used", func() {
			Context("and the virtual guest is only connected to the private network", func() {
				...
				It("generates the correct interfaces", func() {
					...
					Expect(interfaces).To(ConsistOf([]vm.Interface{{
						Name:           "eth0",
						Auto:           true,
						AllowHotplug:   true,
						...
						Gateway:        "10.155.248.161",
						DefaultGateway: true,
						Routes:         vm.SoftlayerPrivateRoutes("10.155.248.161"),
					}}))
			...
			Context("and manual networks are present", func() {
				BeforeEach(func() {
					networks = vm.Networks{
						...
						"private-another-manual": vm.Network{
							Type:    "manual",
							IP:      "10.155.248.130",
							Netmask: "255.255.255.240",
							Gateway: "10.155.248.129",
						},
						"public-manual": vm.Network{
							Type:    "manual",
							IP:      "169.45.188.210",
							Netmask: "255.255.255.240",
							Gateway: "169.45.188.209",
				...
				It("generates all interfaces", func() {
					...
					Expect(interfaces).To(ConsistOf([]vm.Interface{{
						...
						Netmask:        "255.255.255.192",
						Gateway:        "10.155.198.1",
						Routes:         vm.SoftlayerPrivateRoutes("10.155.198.1"),
						DefaultGateway: false,
					}, {
						...
						Netmask:        "255.255.255.240",
						Gateway:        "10.155.248.129",
						Routes:         vm.SoftlayerPrivateRoutes("10.155.248.129"),
						DefaultGateway: false,
					}, {
