...
var _ = Describe("RegistryAgentEnvService", func() {
	var (
		...
		agentEnvService      bslcvm.AgentEnvService
		registryServer       *registryServer
		expectedAgentEnv     bslcvm.AgentEnv
	...
	BeforeEach(func() {
		...
		registryOptions := bslcvm.RegistryOptions{
			Host:     "127.0.0.1",
			Port:     6307,
		...
		agentEnvService = bslcvm.NewRegistryAgentEnvService(registryOptions, instanceID, logger)

		expectedAgentEnv = bslcvm.AgentEnv{AgentID: "fake-agent-id"}
		var err error
	...
	Describe("Fetch", func() {
		...
		Context("when settings for instance do not exist", func() {
			It("returns an error", func() {
				...
				Expect(agentEnv).To(Equal(bslcvm.AgentEnv{}))
			})
		})
	})
...
type registryServer struct {
	...
	options          bslcvm.RegistryOptions
	listener         net.Listener
}

func NewRegistryServer(options bslcvm.RegistryOptions) *registryServer {
