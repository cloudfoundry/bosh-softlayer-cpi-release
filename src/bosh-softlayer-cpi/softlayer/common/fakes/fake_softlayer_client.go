...
type FakeSoftLayerClient struct {
	...
	doRawHttpRequestReturnsOnCall map[int]struct {
		result1 []byte
		result2 int
		result3 error
	}
...
func (fake *FakeSoftLayerClient) DoRawHttpRequest(path string, requestType string, requestBody *bytes.Buffer) ([]byte, int, error) {
	...
	ret, specificReturn := fake.doRawHttpRequestReturnsOnCall[len(fake.doRawHttpRequestArgsForCall)]
...
func (fake *FakeSoftLayerClient) DoRawHttpRequestReturnsOnCall(i int, result1 []byte, result2 int, result3 error) {
	fake.DoRawHttpRequestStub = nil
	if fake.doRawHttpRequestReturnsOnCall == nil {
		fake.doRawHttpRequestReturnsOnCall = make(map[int]struct {
			result1 []byte
			result2 int
			result3 error
		})
	}
	fake.doRawHttpRequestReturnsOnCall[i] = struct {
		result1 []byte
		result2 int
		result3 error
	}{result1, result2, result3}
