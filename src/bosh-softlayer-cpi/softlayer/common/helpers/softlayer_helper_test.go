...
var _ = Describe("SoftLayerVirtualGuest", func() {
	...
	BeforeEach(func() {
		...
		api.POLLING_INTERVAL = 1 * time.Millisecond
	})
	...
	Describe("WaitForVirtualGuestLastCompleteTransaction", func() {
		Context("where the transcation is not completed successfully", func() {
			...
			It("returns nil", func() {
				err := slhelper.WaitForVirtualGuestLastCompleteTransaction(fakeSoftLayerClient, 1234567, "Service Setup")
	...
	Describe("AttachEphemeralDiskToVirtualGuest", func() {
		Context("where the local disk is attached successfully", func() {
			...
			It("returns nil", func() {
				err := slhelper.AttachEphemeralDiskToVirtualGuest(fakeSoftLayerClient, 12345, 25, logger)
				Expect(err).NotTo(HaveOccurred())
			})
		})
